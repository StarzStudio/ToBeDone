...
enum ResultItemState {
    case success
    case failure(FailureTypesItemState)
}

enum FailureTypesItemState {
    case noTitleProvided
}
...
//    }
    ...
    func saveState() -> ResultItemState {
            if itemTitle == "" {
                return ResultItemState.failure(.noTitleProvided)
            } else {
                localServiceStore()
                remoteServiceStore()
                clean()
                return ResultItemState.success
            }
    }
    
    private func localServiceStore() {
        // add item into local  database
        todoItemStore.add(item: item)
    }
    
    private func remoteServiceStore() {
